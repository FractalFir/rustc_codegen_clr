...
use core::cell::{Cell, Ref, RefCell, RefMut, SyncUnsafeCell, UnsafeCell};
    ...
    pub fn pad_integral(&mut self, is_nonnegative: bool, prefix: &str, buf: &str) -> Result {
        ...
        match self.width {
            ...
            Some(min) if self.sign_aware_zero_pad() => {
                let old_fill = core::mem::replace(&mut self.fill, '0');
                let old_align = core::mem::replace(&mut self.align, rt::Alignment::Right);
                write_prefix(self, sign, prefix)?;
...
pub struct EscapeDebugExtArgs {
    /// Escape Extended Grapheme codepoints?
    pub escape_grapheme_extended: bool,
