...
enum VerificationFailure {
    ...
    StackUnexpected2{expected:Type,got:Type},
    OpUnsuported(Type),
...
fn equivalent(a: &Type, b: &Type) -> bool {
    ...
    match (a, b) {
        ...
        (Type::ISize | Type::USize | Type::Ptr(_), Type::ISize | Type::USize | Type::Ptr(_)) => true,
        _ => false,
    }
}
fn verify_op(
    ...
    match op {
        ...
        CILOp::Shl => {
            ...
            let b = stack.pop().ok_or(VerificationFailure::StackUnderflow)?;
            match a {
                Type::U8 | Type::U16 | Type::U32 => Ok(()),
                Type::I8 | Type::I16 | Type::I32 => Ok(()),
                Type::U64 | Type::I64 | Type::USize | Type::ISize => Ok(()),
                _ => Err(VerificationFailure::OpUnsuported2(a.clone(), b.clone())),
            }?;
            match b {
                Type::U8 | Type::U16 | Type::U32 => Ok(stack.push(Type::U32)),
                Type::I8 | Type::I16 | Type::I32 => Ok(stack.push(Type::I32)),
        ...
        CILOp::Neg => {
            ...
            match a {
                Type::U8 | Type::U16 | Type::U32 => Ok(stack.push(Type::U32)),
                Type::I8 | Type::I16 | Type::I32 => Ok(stack.push(Type::I32)),
                Type::U64 | Type::I64 | Type::USize | Type::ISize | Type::F32 | Type::F64 => Ok(stack.push(a)),
                _ => Err(VerificationFailure::OpUnsuported(a)),
            }
        }
        ...
        CILOp::Call(call_info) |  CILOp::CallVirt(call_info)=> {
            for(index,input) in call_info.inputs().iter().rev().cloned().map(to_stacktype).enumerate() {
                let stack_input = stack.pop().ok_or(VerificationFailure::StackUnderflow)?;
                if !equivalent(&input, &stack_input) {
                    if index == 0 && !call_info.is_static() && matches!(input,Type::USize | Type::ISize){
                        continue;
                    }
        ...
        CILOp::STIndI8 | CILOp::STIndI16 | CILOp::STIndI32 => {
            ...
            if !matches!(ptr, Type::Ptr(_) | Type::USize | Type::ISize | Type::ManagedReference(_)){
                return Err(VerificationFailure::StackUnexpected(
                    ptr,
                ));
            }
        ...
        CILOp::LDField(filed_desc)=>{
            let stack_input = stack.pop().ok_or(VerificationFailure::StackUnderflow)?;
            if matches!(stack_input,Type::USize | Type::ISize | Type::ManagedReference(_) | Type::Ptr(_)) || Some(filed_desc.owner()) == stack_input.as_dotnet().as_ref(){
        ...
        CILOp::STField(filed_desc) => {
            let stack_input = stack.pop().ok_or(VerificationFailure::StackUnderflow)?;
            let ptr = stack.pop().ok_or(VerificationFailure::StackUnderflow)?;
            if !matches!(ptr,Type::USize | Type::ISize | Type::ManagedReference(_) | Type::Ptr(_)) || Some(filed_desc.owner()) == stack_input.as_dotnet().as_ref(){

                return Err(VerificationFailure::PointerExpected(
                    ptr,
                ));
            }
