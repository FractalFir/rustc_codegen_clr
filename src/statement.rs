...
pub fn handle_statement<'tcx>(
    ...
    let res = match kind {
        ...
        StatementKind::Assign(palce_rvalue) => {
            ...
            let rvalue_ops =
                crate::rvalue::handle_rvalue(rvalue, tyctx, &place, method, method_instance,type_cache);
            crate::place::place_set(&place, tyctx, rvalue_ops, method, method_instance,type_cache)
        }
        StatementKind::Intrinsic(non_diverging_intirinsic) => {
            match non_diverging_intirinsic.as_ref() {
                ...
                NonDivergingIntrinsic::CopyNonOverlapping(CopyNonOverlapping {
                    ...
                    let dst_op =
                        crate::operand::handle_operand(dst, tyctx, method, method_instance,type_cache);
                    let src_op =
                        crate::operand::handle_operand(src, tyctx, method, method_instance,type_cache);
                    let count_op =
                        crate::operand::handle_operand(count, tyctx, method, method_instance,type_cache);
