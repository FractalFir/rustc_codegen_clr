use crate::{r#type::{Type, TyCache}, method::Method, assembly::Assembly, function_sig::FnSig, access_modifier::AccessModifer, cil::{CILOp, FieldDescriptor}};
use rustc_middle::
    ty::{List, Ty, TyCtxt, TyKind}
;
pub fn add_caller_location(asm: &mut Assembly,tyctx:TyCtxt,cache:&mut TyCache){
    let panic_location = tyctx. get_lang_items(()).get(rustc_hir::lang_items::LangItem::PanicLocation).expect("ERROR: could not find PanicLocation!");
    let adt_def = tyctx.adt_def(panic_location);
    let panic_location = Ty::new(tyctx, TyKind::Adt(adt_def, List::identity_for_item(tyctx,panic_location)));
