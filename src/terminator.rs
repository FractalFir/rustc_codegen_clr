...
use crate::{
    ...
    function_sig::FnSig, operand::handle_operand, r#type::DotnetTypeRef,
};
fn call<'ctx>(
    fn_type: &Ty<'ctx>,
    body: &'ctx Body<'ctx>,
...
pub fn handle_terminator<'ctx>(
    ...
    match &terminator.kind {
        ...
        TerminatorKind::Return => if crate::r#type::Type::from_ty(method.return_ty(),tyctx) != crate::r#type::Type::Void{
        ...
        TerminatorKind::Assert{cond,expected,msg,target,unwind}=>{
            let mut ops = handle_operand(cond, tyctx, method);
            ops.push(CILOp::LdcI32(*expected as i32));
            ops.push(CILOp::BEq(target.as_u32()));
            ops.extend(throw_assert_msg(msg,tyctx,method));
            ops
        }
...
fn throw_assert_msg<'ctx>(msg:&rustc_middle::mir::AssertMessage<'ctx>,tyctx: TyCtxt<'ctx>,method: &rustc_middle::mir::Body<'ctx>)->Vec<CILOp>{
    use rustc_middle::mir::AssertKind;
    match msg{
        AssertKind::BoundsCheck{len,index}=>{
            let mut ops = Vec::with_capacity(8);
            ops.push(CILOp::LdStr("index out of bounds: the len is ".into()));
            ops.extend(handle_operand(len, tyctx, method));
            ...
            ops.extend(handle_operand(index, tyctx, method));
            ops.push(CILOp::Call(usize_to_string.clone()));

            let sig = FnSig::new(&[string_type.clone(),string_type.clone(),string_type.clone(),string_type.clone()],&crate::r#type::Type::Void);
            let out_of_range_exception = DotnetTypeRef::new(Some("System.Runtime"),"System.IndexOutOfRangeException");
            ops.push(CILOp::NewObj(CallSite::boxed(Some(string_class), ".ctor".into(), sig, false)));
            let sig = FnSig::new(&[string_type],&crate::r#type::Type::Void);
            ops.push(CILOp::NewObj(CallSite::boxed(Some(out_of_range_exception), ".ctor".into(), sig, false)));
        ...
        AssertKind::DivisionByZero(operand)=>{
            ...
            let div_by_zero_exception = DotnetTypeRef::new(Some("System.Runtime"),"System.DivideByZeroException");
            ops.push(CILOp::NewObj(CallSite::boxed(Some(div_by_zero_exception), ".ctor".into(), sig, false)));
            ops.push(CILOp::Throw);
            ops
        }
        ...
        AssertKind::Overflow(binop,a,b)=>{
            let mut ops = Vec::with_capacity(8);
            let string_class = DotnetTypeRef::new(Some("System.Runtime"),"System.String");
            ops.push(CILOp::LdStr(format!("attempt to {binop:?} with overflow lhs:").into()));
            ops.extend(handle_operand(a, tyctx, method));
            let usize_class = DotnetTypeRef::new(Some("System.Runtime"),"System.UIntPtr");
            let string_type = crate::r#type::Type::DotnetType(Box::new(DotnetTypeRef::new(Some("System.Runtime"),"System.String")));
            let sig = FnSig::new(&[],&string_type);
            let usize_to_string = CallSite::boxed(Some(usize_class), "ToString".into(), sig, false);
            ops.push(CILOp::Call(usize_to_string.clone()));
            ops.push(CILOp::LdStr("rhs:".into()));
            ops.extend(handle_operand(b, tyctx, method));
            ops.push(CILOp::Call(usize_to_string.clone()));

            let sig = FnSig::new(&[string_type.clone(),string_type.clone(),string_type.clone(),string_type.clone()],&crate::r#type::Type::Void);
            ops.push(CILOp::NewObj(CallSite::boxed(Some(string_class), ".ctor".into(), sig, false)));
            let sig = FnSig::new(&[string_type],&crate::r#type::Type::Void);
            let ovefow_exception = DotnetTypeRef::new(Some("System.Runtime"),"System.ArithmeticException");
            ops.push(CILOp::NewObj(CallSite::boxed(Some(ovefow_exception), ".ctor".into(), sig, false)));
            ops.push(CILOp::Throw);
            ops
        }
