use cilly::{
    call, cil_node::CILNode, cilnode::MethodKind, eq, gt, gt_un, ld_field, lt, lt_un, Assembly, ClassRef, FieldDesc, FieldIdx, Float, Int, MethodRef, Type
};
    ...
    match ty_a.kind() {
        ...
        | TyKind::FnPtr(_, _) => {
            if is_fat_ptr(ty_a,ctx.tcx(), ctx.instance()){
                ...
                let f0 = FieldIdx::data_ptr(ctx,tpe);
                let f0 = eq!(ld_field!(operand_a.clone(),f0),ld_field!(operand_b.clone(),f0));
                let f1 = FieldIdx::metadata(ctx,tpe);
