...
fn run_test_impl(test_id:u64,is_release:bool) ->Option<f64> {
    ...
    let dotnet_out = rustc_codegen_clr::compile_test::test_dotnet_executable(&dotnet_wrapper, &test_dir);
    ...
    let rust_out =
        std::process::Command::new(&native_exec)
...
fn gen_file(test_id:u64,generator:&str){
    let rust_src = format!("/tmp/fuzz/fuzz{test_id}.rs");
    let cout = Command::new(generator).arg("-p").arg(&format!("{test_id}")).output().unwrap();
    assert!(cout.stderr.is_empty());
    let src = cout.stdout;
    let mut file = std::fs::File::create(rust_src).unwrap();
    file.write_all(b"#![allow(dead_code,unused_variables)]").unwrap();
